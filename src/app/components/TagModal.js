...
const TagModal = ({ oeuvreId, onClose, onTagUpdate }) => {
  ...
  useEffect(() => {
    const fetchOeuvreAndTags = async () => {
      try {
        ...
        const oeuvreRes = await fetch(`/api/proxy/oeuvres/${oeuvreId}?populate=tags`);
        ...
        const tagRes = await fetch(`/api/proxy/tags`);
  ...
  const handleCreateTag = async () => {
    if (!tagSearchTerm.trim()) {
      toast.warning("Le nom du tag ne peut pas être vide.");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/proxy/tags`, {
        ...
        body: JSON.stringify({ data: { nom: tagSearchTerm } }),
      });
  ...
  const handleToggleTag = async (tagDocumentId) => {
    ...
    try {
      ...
      await fetch(`/api/proxy/oeuvres/${oeuvre.documentId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ data: { tags: updatedTags.map((tag) => tag.documentId) } }),
      });
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
      <div role="dialog" aria-modal="true" aria-label="Gérer les tags" className="p-6 bg-gray-900 text-white rounded shadow-lg w-full max-w-4xl">
