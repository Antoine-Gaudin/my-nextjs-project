...
export default function Teams({ user }) {
  ...
  const fetchTeams = async () => {
    ...
    setErrorMsg("");
    try {
      // Récupérer en parallèle les teams (owner + membre) et les invitations
      const [ownerRes, memberRes, invitesRes] = await Promise.all([
        fetch(
      ...
      const [ownerData, memberData, invitesData] = await Promise.all([
        ownerRes.json(),
        memberRes.json(),
        invitesRes.json(),
      ]);
